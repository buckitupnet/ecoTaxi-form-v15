...
- Управления локальным зашифрованным хранилищем через библиотеку `@lo-fi/local-vault`.
- Создания нового или подключения к существующему хранилищу.
- Шифрования и расшифровки данных с использованием passkey.
- Поддержания состояния авторизации и уведомления об изменениях через события.
- Удобного интерфейса для хранения, извлечения и удаления данных.
- Обработки ошибок и управления отменой операций.
...
- **События**: Реализует `EventTarget` для подписки на изменения состояния авторизации.
...
Класс реализует интерфейс `EventTarget` для подписки на изменения состояния:

- **`authChange`**:  
  Срабатывает при изменении состояния авторизации.
   - **Данные события**: `{ isAuth: boolean }`.
...
manager.addEventListener("authChange", (event) => {
   console.log("Авторизация изменена:", event.detail.isAuth);
});
```
...
Исправленная документация с добавленными изменениями в методах и подпиской на события:
